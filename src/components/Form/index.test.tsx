...
describe('Form Component', () => {
  ...
  test('element text regression test', () => {
    ...
    expect(wrapper.find('[htmlFor="select-addresses"]').text()).toBe(
      'Last used address'
    );
    expect(wrapper.find('[htmlFor="select-network"]').text()).toBe('Network');
    expect(wrapper.find('#select-addresses option').length).toEqual(1);
    expect(wrapper.find('#select-addresses option:first-child').text()).toBe(
      'Select from previous address'
    );
    expect(wrapper.find('#select-network option').length).toEqual(2);
    expect(wrapper.find('#select-network option:first-child').text()).toBe(
      'Rinkeby'
    );
    expect(wrapper.find('#select-network option:last-child').text()).toBe(
      'Mainnet'
    );
  ...
  test('input address work correctly', () => {
    ...
    wrapper
      .find('#input-address')
      .simulate('change', { target: { value: 'thecorn-address-code' } });
    expect(wrapper.find('#input-address').props().value).toBe(
      'thecorn-address-code'
    );
  ...
  test('submit work correctly', () => {
    ...
    wrapper
      .find('#select-network')
      .simulate('change', { target: { value: 'mainnet' } });
